...
from examples.nlp4if.arabic.bb_arabert_v2_tokenized.arabic_bb_arabert_v2_t_config import TEMP_DIRECTORY, MODEL_NAME, \
    config, \
    MODEL_TYPE, SEED, SUBMISSION_FILE
...
train = pd.read_csv(os.path.join("examples", "arabic", "data", "covid19_disinfo_binary_arabic_train.tsv"), sep='\t')

train.dropna(subset=["q5_label"], inplace=True)

# Class count
count_class_no, count_class_yes = train.q5_label.value_counts().sort_index(ascending=True)

# Divide by class
df_class_no = train[train['q5_label'] == "no"]
df_class_yes = train[train['q5_label'] == "yes"]

size_counter = sample_size_counter(count_class_no, count_class_yes, 1)
print("NOs : ", df_class_no['q5_label'].count())
print("YESs : ", df_class_yes['q5_label'].count())
print("size counter : ", size_counter)

if size_counter > 0:

    df_class_no_under = df_class_no.sample(count_class_yes * size_counter)
    print("under sized NOs : ", df_class_no_under['q5_label'].count())
    ...
    print("under sized YESs : ", df_class_yes_under['q5_label'].count())
    train = pd.concat([df_class_yes_under, df_class_no], axis=0)

train['labels'] = encode(train["q5_label"])
